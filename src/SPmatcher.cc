...
namespace ORB_SLAM2
{
	...
	int SPmatcher::SearchByProjection(KeyFrame* pKF, cv::Mat Scw, const vector<MapPoint*>& vpPoints, vector<MapPoint*>& vpMatched, int th)
	{
	...
	int SPmatcher::Fuse(KeyFrame* pKF, const vector<MapPoint*>& vpMapPoints, const float th)
	{
	...
	int SPmatcher::Fuse(KeyFrame* pKF, cv::Mat Scw, const vector<MapPoint*>& vpPoints, float th, vector<MapPoint*>& vpReplacePoint)
	{
		// Get Calibration Parameters for later projection
		const float& fx = pKF->fx;
		const float& fy = pKF->fy;
	...
	int SPmatcher::SearchBySim3(KeyFrame* pKF1, KeyFrame* pKF2, vector<MapPoint*>& vpMatches12,
	...
	{
		// 将汉明距离修改为L2范数，同时将该文件中其他距离的数据类型从int改为float
